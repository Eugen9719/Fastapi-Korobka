    ...
    async def test_create_stadium(self, db: AsyncSession, user_id, slug, expected_exception, status_code, detail):
        ...
        if expected_exception:
            with pytest.raises(expected_exception) as exc_info:
                await service_factory.stadium_service.create_stadium(db=db, schema=create_schema, user=user)
            assert exc_info.value.status_code == status_code
            assert exc_info.value.detail == detail
        else:
            stadium = await service_factory.stadium_service.create_stadium(db=db, schema=create_schema, user=user)
