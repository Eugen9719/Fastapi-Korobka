...
async def add_review(db: TransactionSessionDep, user: CurrentUser, schema: CreateReview, stadium_id: int):
    ...
    """
    return await service_factory.review_service.create_review(db=db, schema=schema, stadium_id=stadium_id, user=user)
...
async def update_review(schema: UpdateReview, db: TransactionSessionDep, user: CurrentUser, review_id: int):
    ...
    """
    return await service_factory.review_service.update_review(db, schema=schema, user=user, review_id=review_id)
...
async def delete_review(db: TransactionSessionDep, review_id: int, user: CurrentUser) -> Msg:
    ...
    """
    return await service_factory.review_service.delete_review(db, user=user, review_id=review_id)
